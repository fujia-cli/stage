...
export class CliInit extends CliCommand {
  ...
  async downloadTemplate() {
    ...
    if (this.templateType === 'default') {
      ...
      if (!selectTemplate)
        throw new Error('The selected template is not exist!');

      const templateInfo = this.template.find(
        (t) => t.npmName === selectTemplate,
      );
  ...
  async installTemplate() {
    if (!this.templateInfo)
      throw new Error("The info of template isn't exist.");
  ...
  async installProjectTemplate(templatePath: string) {
    const { installCommand, startCommand, name } = this
      .templateInfo as ProjectTemplate;
