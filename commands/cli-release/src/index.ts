...
export class ReleaseCommand extends CliCommand {
    ...
    */
    try {
      ...
      if (!pkgInfo?.files && !isExistGitIgnore) {
        throw new Error('please make sure have the "files" field in package.json or have .gitignore file in the current working directory');
      }
  ...
  async checkMainBranch() {
    ...
    if (!['main', 'master'].includes(branch.current)) {
      throw new Error(`the current git branch is not main or master, please run the command: "git checkout main", then try again!`);
    }
  ...
  async checkNpmRegistry() {
    ...
    const npmRcObj = readDotFileToObj(npmRcPath);
    ...
    if (registry && registry !== NPM_REGISTRY) {
      ...
      log.success('[cli-release]', `run "npx nrm use npm" successful and current npm registry:  https://registry.npmjs.org/`)
    }
  }
